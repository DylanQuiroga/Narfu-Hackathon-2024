...
async function fetchWeatherData() {
    try {
        const API_KEY = 'API_KEY'; // Reemplaza con tu API Key de OpenWeatherMap
        const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=Ovalle&appid=${API_KEY}&units=metric&lang=es`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
